#!/usr/bin/env python3
"""Upload a markdown article (and local images) to WordPress via REST API."""
...
class WordPressUploader:
    ...
    def create_post(self, title: Optional[str], content: str, categories: List[int], status: str) -> dict:
        payload = {
            "content": content,
            "status": status,
...
def find_local_images(markdown_text: str, article_path: Path) -> List[ImageRef]:
    refs: List[ImageRef] = []
...
def convert_html_to_blocks(html_content: str, media_info: Dict[str, dict]) -> str:
    ...
    for node in soup.contents:
        block = convert_node_to_block(node, media_info)
        if block:
            blocks.append(block)
    return "\n\n".join(blocks)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Upload markdown article to WordPress.")
    parser.add_argument(
        "--markdown",
        default="articles/1/chatgpt_partner_article.md",
    ...
    parser.add_argument(
        "--skip-title",
        action="store_true",
        help="Do not send the markdown H1 as the WordPress title.",
    )
...
def main() -> int:
    ...
    if not article_path.exists():
        raise SystemExit(f"Markdown file not found: {article_path}")
    ...
    post = uploader.create_post(title=title, content=block_content, categories=category_ids, status=args.status)
